...
function DocumentConversion(options, comments) {
    ...
    var defaultParagraphStyle = function(attrs) {
        return htmlPaths.topLevelElement("p", attrs);
    };
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        if (style) {
            var align = !element.alignment && /[HEADING|TITLE]/.test(element.styleName.toLocaleUpperCase()) ? 'center' : element.alignment;

            var cloneStyle = clone(style);
            cloneStyle.to._elements.forEach(function(el) {
                el.attributes.align = align;
            });

            return cloneStyle.to;
        } else {
            if (element.styleId) {
            ...
            if (element.alignment) {
                attrs.align = element.alignment;
            }
            return defaultParagraphStyle(attrs);
        }
    }
    ...
    function convertRun(run, messages, options) {
        ...
        if (run.color || run.highlight) {
            var color = run.color ? "color:" + run.color : "";
            var highlight = run.highlight ? "background-color:" + run.highlight : "";
            paths.push(htmlPaths.element("font", {style: [color, highlight].join(';')}, {fresh: false}));
        }
    ...
    function convertRuby(element, messages, options) {
        var children = convertElements(element.children, messages, options);

        // Arrange the children in the order of rb, rt, rb, rt, ...
        var arrangedChildren = [];
        var baseChildren = children.filter(function(child) {
            return child.tag.tagName === "rb";
        });
        var textChildren = children.filter(function(child) {
            return child.tag.tagName === "rt";
        });
        for (var i = 0; i < baseChildren.length; i++) {
            arrangedChildren.push(baseChildren[i]);
            if (i < textChildren.length) {
                arrangedChildren.push(textChildren[i]);
            }
        }
        return [
            Html.freshElement("ruby", {}, [Html.forceWrite].concat(arrangedChildren))
        ];
    }

    function convertRubyText(element, messages, options) {
        var children = convertElements(element.children, messages, options);
        return [
            Html.freshElement("rt", {}, [Html.forceWrite].concat(children))
        ];
    }

    function convertRubyBase(element, messages, options) {
        var children = convertElements(element.children, messages, options);
        return [
            Html.freshElement("rb", {}, [Html.forceWrite].concat(children))
        ];
    }

    function convertMath(element, messages, options) {
        function convertDomToObj(el) {
            var node = {
                type: {
                    1: 'element',
                    3: 'text'
            ...
            if (el.childNodes.length) {
                ...
                children.forEach(function(item) {
                    node.children.push(convertDomToObj(item));
                });
            }
            return node;
        }
        ...
        var doc = new xmldom.DOMParser().parseFromString(
            ['<w:document xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math">', element.value, "</w:document>"].join('')
        );

        var mathmlElement = omml2mathml(doc);
        var nodes = convertDomToObj(mathmlElement);
    ...
    var elementConverters = {
        ...
        "break": convertBreak,
        "ruby": convertRuby,
        "rubyText": convertRubyText,
        "rubyBase": convertRubyBase,
        "math": convertMath
    };
    return {
