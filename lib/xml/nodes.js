...
Element.prototype.traverseFilter = function(name) {
    var elements = _.reduce(this.children, function(pre, current) {
        if (current.children && current.children.length) {
            return pre.concat(current, current.traverseFilter(name));
        }
        return pre.concat(current);
    }, []);
    elements = _.filter(elements, function(child) {
        return child.name === name;
    });
    return elements;
};
