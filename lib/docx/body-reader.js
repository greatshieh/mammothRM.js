...
function BodyReader(options) {
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            ...
            return {
                ...
                color: !!element.first("w:color") && ('#' + element.first("w:color").attributes['w:val']),
    ...
    var xmlElementReaders = {
        ...
        "v:group": function(element) {
            // support external source image width/height
            if (element.attributes.coordsize && element.attributes.style) {
                var coordsize = element.attributes.coordsize.split(',');
                var groupStyle = element.attributes.style;
                var groupWidth = groupStyle.match(/width:(\d+\.?\d{0,2})pt;/)[1];
                var groupHeight = groupStyle.match(/height:(\d+\.?\d{0,2})pt;/)[1];
                var widthRatio = coordsize[0] / groupWidth;
                var heightRatio = coordsize[1] / groupHeight;
                element.traverseFilter('v:shape').forEach(function(el) {
                    var style = el.attributes.style ? el.attributes.style + ";" : "";
                    var width = style.match(/width:(\d+\.?\d{0,2});/)[1];
                    var height = style.match(/height:(\d+\.?\d{0,2});/)[1];

                    el.firstOrEmpty('v:imagedata').attributes.width = width / widthRatio;
                    el.firstOrEmpty('v:imagedata').attributes.height = height / heightRatio;
                });
            }
        ...
        "w:ruby": readRuby,
        "w:rt": readRubyText,
        "w:rubyBase": readRubyBase,
        'm:oMathPara': function(element) {
            return elementResult(
                new documents.Math(element.serializeToString())
            );
        },
    ...
    function readImageData(element) {
        ...
        if (relationshipId) {
            var image = findEmbeddedImageFile(relationshipId);
            image.width = String(element.attributes.width || "");
            image.height = String(element.attributes.height || "");
    ...
    function readRuby(element) {
        return readXmlElements(element.children).map(function(children) {
            return documents.Ruby(children);
        });
    }

    function readRubyText(element) {
        return readXmlElements(element.children).map(function(children) {
            return documents.RubyText(children);
        });
    }

    function readRubyBase(element) {
        return readXmlElements(element.children).map(function(children) {
            return documents.RubyBase(children);
        });
    }
